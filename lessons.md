...
- Fix: Removed Compute3 provider entirely. All agents now use OpenRouter models only (stepfun/step-3.5-flash, qwen/qwen3-coder). Do NOT re-add Compute3.
- Follow-up: Always configure critical agents with multiple providers when possible; monitor provider health.
...
- Follow-up: Any change to nav, footer, or shared CSS in build.js MUST also be applied to all 10 hand-crafted pages (see TOOLS.md for full list). deploy-prod.sh now includes permissions fix and cache purge.
